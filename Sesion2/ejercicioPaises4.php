...
//RECORRER EL ARRAY para añadir la capital
foreach ($paises as $pais) {
    //Inicializo las variables para definir el tipo
    $capital="";
    $capital=sacarCapital($pais->nombre);
    $pais->capital=$capital;
}

//Asigno el array antiguo al nuevo
$nuePaises=$paises;
foreach ($nuePaises as $pais) {
//Vamos añadiendo fila a fila
    $sHtml.="<tr>";
    $sHtml.="   <td class=\"alineadoIzq\">$pais->nombre</td>";
    $sHtml.="   <td>$pais->capital</td>";
    $sHtml.="   <td>$pais->habitantes</td>";
    $sHtml.="   <td>$pais->superficie</td>";
    $sHtml.="</tr>";
}
